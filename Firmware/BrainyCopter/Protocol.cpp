...
void evaluateCommand(uint8_t c) {
  ...
  switch(c) {
   ...
   case MSP_READ_TEST_PARAM:
     ...
     paramList[1] = conf.pid[PIDALT].P8 * 250.f / 200;
     paramList[2] = conf.pid[PIDALT].I8;
     paramList[3] = conf.pid[PIDALT].D8 * 250.f / 100;
   ...
   case MSP_SET_TEST_PARAM:
     ...
     conf.pid[PIDALT].P8 = paramList[1] / 250.f * 200;   //0~200
     conf.pid[PIDALT].I8 = paramList[2];                 //0~250
     conf.pid[PIDALT].D8 = paramList[3] / 250.f * 100;   //0~100
     writeParams(0);
     return;
     break;
   ...
   case MSP_HEX_NANO:
     ...
     serialize16((int16_t)alt.EstAlt);
     for(uint8_t i=0;i<2;i++) serialize16(att.angle[i]);
