...
public class OSDData {
	...
	{
		Log.d(TAG,"flightState: "+flightState+", "+(flightState & 0x80));
		return (flightState & 0x80) != 128;
	}
	...
	private void evaluateCommand(byte cmd_, int dataSize){    
	    ...
	    switch(icmd) {
	        ...
	        case OSDCommon.MSP_HEX_NANO:
	            flightState = read8();
				read16();  //throttle, absolutedAccZ
	            altitude = read16();    //EstAlt
	            angleX = read16()/10;  //[-180,180]
	            angleY = read16()/10;  //[-180,180]
	            head = read16();   //AltHold
	            vBat = read8() / 256.0f * 5;  //vbat
				read8(); //pitch trim
				read8(); //roll trim
				Log.d(TAG, "flightState: " + flightState);
				Log.d(TAG, "rcThrottle: " + rcThrottle);
				Log.d(TAG, "altitude: " + altitude);
				Log.d(TAG, "Roll: " + angleX);
				Log.d(TAG, "Pitch: " + angleY);
				Log.d(TAG, "head: " + head);
				Log.d(TAG, "vBat: " + vBat);
				Log.d(TAG, "Pitch trim: " + rcPitch);
				Log.d(TAG, "Roll trim: " + rcRoll);
	            if(delegate != null) {
