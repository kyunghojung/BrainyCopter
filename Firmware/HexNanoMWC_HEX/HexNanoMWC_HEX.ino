...
void loop () 
{
  ...
  { // 50Hz
    ...
    if(rcDelayCommand == 20) 
    {
      ...
      {                        // actions during not armed
        ...
        #endif
        else if (rcSticks == THR_HI + YAW_LO + PIT_LO + ROL_CE) // throttle=max, yaw=left, pitch=min
        {
          calibratingA=512;     
        }
        else if (rcSticks == THR_HI + YAW_HI + PIT_LO + ROL_CE) // throttle=max, yaw=right, pitch=min
        {
          f.CALIBRATE_MAG = 1;    
        }
        i=0;
  ...
  for(axis=0; axis < 3; axis++) 
  {
    ...
    { 
      ...
      {
        error = (rcCommand[axis]<<6)/conf.P8[axis] ; // 16 bits is needed for calculation: 500*64 = 32000      16 bits is ok for result if P8>5 (P>0.5)
      }
      else 
      {
