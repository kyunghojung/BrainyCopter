...
import com.hyunnyapp.brainycopter.BrainyCopterApplication;
import com.hyunnyapp.brainycopter.ble.BleConnectinManager;
import com.hyunnyapp.brainycopter.modal.OSDCommon.MSPCommnand;
...
public class Transmitter implements OSDDataDelegate{
	...
	public void transmmitData(byte[] data){
		if (bleConnectionManager != null && bleConnectionManager.isConnected() && data != null){
			if (BrainyCopterApplication.sharedApplicaion().isFullDuplex()) {
	...
	public void osdDataDidUpdateOneFrame() {
		Text debugTextView = BrainyCopterApplication.sharedApplicaion().getDebugTextView();
		
		String debugString = osdData.getAltitude() + ", " + osdData.getAngleX() + ", " + osdData.getAngleY();
		
		debugTextView.setText(debugString);	
		
		BrainyCopterApplication.sharedApplicaion().setCurrentAlt(osdData.getAltitude());
		BrainyCopterApplication.sharedApplicaion().setCurrentAttitude(-osdData.getAngleX(), -osdData.getAngleY(), 0);
	}
}
